...
Scene scene = new Scene(root);
		...
		e_x.setOnAction(e->{
			try {
			double number = Double.parseDouble(exp.getText());
			result.setText(Math.exp(number)+"");
			}
			catch (Exception ex) {
				post.setText("");
				result.setText("Please Enter only numbers");
			}
			});
		...
		x_2.setOnAction(e->{
			...
			result.setText(Math.pow(number, 2)+"");	
                   }
                   catch (Exception ex) {
       				post.setText("");
       				result.setText("Please Enter only numbers");
